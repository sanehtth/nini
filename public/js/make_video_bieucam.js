...
function getDraftKey() {
  const sid = (getEl('story-id','storyId')?.value || '').trim();
...
async function loadStoryManifestIntoUI() {
  ...
  const select = getEl('story-select','storySelect');
  const pathEl = getEl('manifest-path','manifestPath');
  if (!select) return;
...
async function loadSelectedStoryFromManifest() {
  ...
  // Fill form
  const idEl = getEl('story-id','storyId');
  const titleEl = getEl('story-title','storyTitle');
  const contentEl = getEl('story-content','storyText','storyRawText','rawText');
  ...
  if (contentEl) contentEl.value = (storyJson.story || storyJson.story_text || storyJson.content || storyJson.text || storyJson.rawText || '');

  // Auto select participating characters from loaded story
  if (Array.isArray(storyJson.characters)) {
    try {
      if (data.setSelectedCharacterIds) {
        data.setSelectedCharacterIds(storyJson.characters);
      } else {
        const ids = new Set(storyJson.characters.map(x => String(x)));
        document.querySelectorAll('input[type="checkbox"][data-char-id], input[type="checkbox"][name="participant"]').forEach(cb => {
          const id = cb.getAttribute('data-char-id') || cb.value;
          cb.checked = ids.has(String(id));
        });
        const sc = getEl('selected-count', 'storyCharsSelected');
        if (sc) sc.textContent = `Đã chọn: ${ids.size}`;
      }
    } catch (e) {
      console.warn('[XNC] apply story characters failed', e);
    }
  }
  ...
  const loadBtn = getEl('load-story-btn','loadStoryBtn');
...
function buildStoryObjectFromForm() {
  const storyId = (getEl('story-id','storyId')?.value || '').trim();
  const storyTitle = (getEl('story-title','storyTitle')?.value || '').trim();
  const content = (getEl('story-content','storyText','storyRawText','rawText')?.value || '').trim();
...
function splitScenesFromStory() {
  const storyTextEl = getEl('story-content','storyText','storyRawText','rawText');
...
function setupParticipantsUI() {
  // Supports both UIs:
  // - New: participants-grid (cards)
  // - Legacy: participantsList (simple checkboxes)
  const grid = document.getElementById('participants-grid');
  const list = document.getElementById('participantsList') || document.getElementById('participants-list');
  const selectedCountEl = getEl('selected-count', 'storyCharsSelected');
  const searchEl = getEl('story-char-search', 'storyCharSearch');
  const btnSelectAll = getEl('btn-select-all', 'btnSelectAll');
  const btnClear = getEl('btn-clear', 'btnClear');
  ...
  function getSelectedIds() {
    if (grid) {
      return Array.from(grid.querySelectorAll('input[type="checkbox"][data-char-id]'))
        .filter(cb => cb.checked)
        .map(cb => cb.getAttribute('data-char-id'));
    }
    // legacy list
    return Array.from(list.querySelectorAll('input[type="checkbox"][data-char-id],input[type="checkbox"][name="participant"]'))
      .filter(cb => cb.checked)
      .map(cb => cb.getAttribute('data-char-id') || cb.value);
  }

  function setSelectedIds(ids) {
    const set = new Set((ids || []).map(String));
    if (grid) {
      grid.querySelectorAll('input[type="checkbox"][data-char-id]').forEach(cb => {
        cb.checked = set.has(cb.getAttribute('data-char-id'));
      });
    }
    if (list) {
      list.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        const id = cb.getAttribute('data-char-id') || cb.value;
        cb.checked = set.has(String(id));
      });
    }
    updateCount();
  }
  ...
  function updateCount() {
    ...
    selectedCountEl.textContent = `Đã chọn: ${getSelectedIds().length}`;
  }
  ...
  function renderGrid() {
    ...
    grid.innerHTML = characters.map(c => {
      ...
      const label = c.label || c.name || id || '(no-id)';
      const gender = c.gender || '';
      const role = c.role || c.desc || '';
      const meta = [gender, role].filter(Boolean).join(' • ');
      return `
        <div class="p-card" data-char="${escapeHtml(id)}">
          <label class="p-row">
            <input type="checkbox" data-char-id="${escapeHtml(id)}" />
            <div class="p-text">
  ...
  function renderList() {
    if (!list) return;
    // Keep existing structure if already has children (e.g., hardcoded in HTML), only wire events.
    const hasItems = list.querySelectorAll('input[type="checkbox"]').length > 0;
    if (!hasItems) {
      list.innerHTML = characters.map(c => {
        ...
        const meta = [gender, role].filter(Boolean).join(' • ');
        return `
          <label class="char-item" style="display:block; padding:6px 0;">
            <input type="checkbox" name="participant" value="${escapeHtml(id)}" data-char-id="${escapeHtml(id)}" />
            <strong>${escapeHtml(label)}</strong>
            <span style="opacity:.7; margin-left:6px;">${escapeHtml(meta)}</span>
          </label>
        `;
      }).join('');
    }
  ...
  function applySearch(q) {
    ...
    const pred = (c) => {
      ...
      const role = c.role || c.desc || '';
      return norm(`${id} ${label} ${role}`).includes(nq);
    };

    if (grid) {
      grid.querySelectorAll('[data-char]').forEach(card => {
        const id = card.getAttribute('data-char') || '';
        const c = characters.find(x => String(x.id || x.char_id || '') === id);
        card.style.display = (!nq || (c && pred(c))) ? '' : 'none';
      });
    }
    if (list) {
      list.querySelectorAll('label.char-item, label').forEach(row => {
        // try infer id from checkbox
        const cb = row.querySelector('input[type="checkbox"]');
        const id = cb ? (cb.getAttribute('data-char-id') || cb.value) : '';
        const c = characters.find(x => String(x.id || x.char_id || '') === String(id));
        const show = (!nq || (c && pred(c)));
  ...
  if (btnSelectAll) {
    btnSelectAll.addEventListener('click', () => setSelectedIds(characters.map(c => String(c.id || c.char_id || ''))));
  }
  if (btnClear) {
    btnClear.addEventListener('click', () => setSelectedIds([]));
  }

  // expose helpers for other parts
  data.getSelectedCharacterIds = getSelectedIds;
  data.setSelectedCharacterIds = setSelectedIds;
}
...
function applyStoryCharacterSelection(storyObj) {
  try {
    ...
    const count = getEl('selected-count','storyCharsSelected');
    if (count) count.textContent = String(selectedCharacterIds.size);

    // Re-render UI if present
    const grid = getEl('participants-grid','participantsList');
    if (grid) {
