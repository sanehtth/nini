...
function buildOutfitLine(outfit, gender, signatureColors) {
  ...
  if (!signatureColors || !signatureColors.length) return desc;

  if (allowSig) {
    return `${desc} (accent colors may use the character's signature palette: ${signatureColors.join(', ')})`;
  }
  return `${desc} (do NOT recolor this wardrobe item using signature colors)`;
}
