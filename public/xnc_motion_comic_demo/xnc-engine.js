...
class XNCEngine{
  ...
  // Convert coordinates expressed in "stage-local" space (what getPanelRect/getActorRect return)
  // into overlay-local space (CSS left/top for nodes appended into this.overlay).
  // This makes out-of-frame props robust even if overlay is not perfectly aligned with stage.
  _stagePtToOverlay(pt){
    const s = this.stage.getBoundingClientRect();
    const o = this.overlay.getBoundingClientRect();
    // stage-local -> viewport -> overlay-local
    return { x: (s.left + pt.x) - o.left, y: (s.top + pt.y) - o.top };
  }

  _stageRectToOverlay(rect){
    const a = this._stagePtToOverlay({ x: rect.x, y: rect.y });
    return { x: a.x, y: a.y, w: rect.w, h: rect.h };
  }
  ...
  // -------- Coordinate helpers --------
  // getPanelRect/getActorRect/getAnchorPoint return coordinates relative to `this.stage`.
  // But out-of-frame FX nodes are appended into `this.overlay`, so their CSS left/top
  // must be relative to `this.overlay`.
  _stageOffsetToOverlay(){
    const s = this.stage.getBoundingClientRect();
    const o = this.overlay.getBoundingClientRect();
    return { dx: s.left - o.left, dy: s.top - o.top };
  }

  _stagePtToOverlay(pt){
    const {dx, dy} = this._stageOffsetToOverlay();
    return { x: pt.x + dx, y: pt.y + dy };
  }

  _stageRectToOverlay(r){
    const {dx, dy} = this._stageOffsetToOverlay();
  ...
  async throwProp({from, to, propType="sandal", fromAnchor="handR", toAnchor="face", duration=520}){
    ...
    const start = this._stagePtToOverlay(this.getAnchorPoint(from, fromAnchor));
    const hit = this._stagePtToOverlay(this.getAnchorPoint(to, toAnchor));
    ...
    const s = {w: overlayRect.width - 28, h: overlayRect.height - 28};
  ...
  async pull({from, to, grab="collar", duration=560, carryToPanel=null}){
    ...
    const fromPoint = this._stagePtToOverlay(this.getAnchorPoint(from, "handR"));

    const target = this.actors.get(to);
    const targetRect = this._stageRectToOverlay(this.getActorRect(to));
